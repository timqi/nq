...
- **Plan review with Codex**: When in plan mode for non-trivial tasks, before exiting plan mode, use a subagent to run Codex to review the plan. Feed the plan content to Codex and ask it to critique, identify gaps, suggest improvements, and flag potential issues. Incorporate Codex's feedback into the plan before presenting it for user approval.
