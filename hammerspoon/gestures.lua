...
gestures.createPoints = function(str)
    ...
    if str == "←" then
        for i = 0,40,1 do x = x - 1 table.insert(result, Point(x, y)) end
    elseif str == "↑" then
        for i = 0,40,1 do y = y - 1 table.insert(result, Point(x, y)) end
    elseif str == "→" then
        for i = 0,40,1 do x = x + 1 table.insert(result, Point(x, y)) end
    elseif str == "↓" then
        for i = 0,40,1 do y = y + 1 table.insert(result, Point(x, y)) end
    ...
    elseif str == "↱" then
        for i = 0,25,1 do y = y-1 table.insert(result, Point(x, y)) end
        for i = 0,15,1 do x = x+1 table.insert(result, Point(x, y)) end
    elseif str == "↰" then
        for i = 0,25,1 do y = y-1 table.insert(result, Point(x, y)) end
        for i = 0,15,1 do x = x-1 table.insert(result, Point(x, y)) end
